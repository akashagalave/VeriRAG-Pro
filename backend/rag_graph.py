...
def retrieve_from_vectorstore(
    ...
    for doc in docs:
        ...
        if not result.was_clean:
            doc = Document(page_content=result.content, metadata=doc.metadata)
        sanitized_docs.append(doc)

    summary = f"Retrieved {len(sanitized_docs)} chunk(s) from the vector store (hybrid BM25+dense)."
    return [
        ToolMessage(content=summary, tool_call_id=tool_call_id),
        Command(update={"retrieved_docs": (current_docs or []) + sanitized_docs}),
    ]
...
def agent_node(state: RAGState) -> dict:
    ...
    Uses invoke_with_fallback for circuit-breaker resilience.
